...
//private apiUrl = 'http://localhost:8000/api.php'; // URL de l'API; // URL de votre fichier PHP
  ...
  getIntervenants(): Observable<any> {
    const url = `${environment.apiUrl}?action=getIntervenants`;
    return this.http.get<any>(url);
  }

  getIntervenantById(id: string): Observable<any> {
    const url = `${environment.apiUrl}?action=getIntervenantById&id=${id}`;
    return this.http.get<any>(url);
  }
  // getParticipants() :Observable<any> {
  //   return this.http.get<any>(`${environment.apiUrl}?action=user`, { responseType: 'text' as 'json' })
  //   .pipe(
  ...
  // }
  getParticipants(): Observable<any> {
    const url = `${environment.apiUrl}?action=user`;
    return this.http.get<any>(url);
  }
  // getParticipants(): Observable<any> {
   
  //   return this.http.get<any>(`${environment.apiUrl}?action=getIntervenants`, { responseType: 'text' as 'json' })
  //     .pipe(
  //       map(response => {
  ...
  getParticipantById(id: string): Observable<any> {
    const url = `${environment.apiUrl}?action=user&id=${id}`;
    return this.http.get<any>(url);
  }
}
